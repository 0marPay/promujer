...
for indice, usuario_emprende in usuarios_emprende.iterrows():
    ...
    correo_emprente = correo_emprende.replace("'", "")
    print()
    print(f"{indice}/{full_len} - {round(indice/full_len,5)}% - {correo_emprende}")
    
    usuarios_hubspot = pd.read_sql(f"SELECT * FROM usuario_emprende WHERE CorreoElectronico = '{correo_emprende}'", con=connection_emprende_db)
    ...
    # Caso b: Si coincide solo uno, verificar los campos del ID Emprende
        ...
        if (id_emprende_emprende == id_emprende_hubspot) :
        ...
        # Verificar si coinciden todos los campos del ID Emprende
            ...
            emprende_db.execute(f"UPDATE usuario_emprende SET Status = '{status}' WHERE correo = '{correo_emprende}'")
            emprende_db.execute(f"UPDATE usuario_hubspot SET Status = '{status}' WHERE correo = '{correo_emprende}'")
